...
class OrderRouter:
    def __init__(
        ...
        # Order TTL (expires_after): Orders signed with expiration timestamp for safety
        # This prevents orphaned orders if the bot crashes after signing but before receiving ack
        # Default: 60 seconds (60000ms). Set to 0 or None to disable.
        self.order_ttl_ms = getattr(self.cfg, "order_ttl_ms", 60000)
    ...
    def _set_order_expiry(self) -> None:
        """
        Set expires_after on exchange for order TTL safety.
        
        This ensures orders are rejected by the exchange if they arrive too late,
        preventing orphaned orders from persisting if bot crashes after signing
        but before receiving acknowledgment.
        """
        if self.order_ttl_ms and self.order_ttl_ms > 0:
            expires_at = int(time.time() * 1000) + self.order_ttl_ms
            self.exchange.set_expires_after(expires_at)
        else:
            # Disable expiry
            self.exchange.set_expires_after(None)

    def _clear_order_expiry(self) -> None:
        """Clear expires_after after order submission."""
        self.exchange.set_expires_after(None)

    async def _flush(self, batch: list[tuple[OrderRequest, asyncio.Future]]) -> None:
        order_type = {"limit": {"tif": "Gtc"}}
        async with self.nonce_lock:
            # Set order TTL before submitting
            self._set_order_expiry()
            try:
                if len(batch) > 1:
                    for i in range(0, len(batch), self.max_bulk):
                        await self._flush_bulk(batch[i : i + self.max_bulk], order_type)
                    return
                await self._flush_single(batch[0], order_type)
            finally:
                # Always clear expiry after submission attempt
                self._clear_order_expiry()

    async def _flush_single(self, item: tuple[OrderRequest, asyncio.Future], order_type: dict) -> None:

...
import math
import time
from typing import Any, Callable, Deque, Dict, Optional, Set, TypeVar
...
def safe_call(
    ...
    """
    Execute a function safely, logging any exceptions without raising.
    
    Use for non-critical operations (metrics, events, cleanup) that should
    not break main execution flow but still need error visibility.
    
    Args:
        func: Zero-argument callable to execute
        context: Description of what operation is being attempted (for logging)
        logger: Logger to use; if None, uses module logger
        default: Value to return on exception
        
    Returns:
        Result of func() on success, default on exception
        
    Example:
        safe_call(lambda: metrics.inc("fills"), "update fill metrics", self.logger)
    """
    _logger = logger or logging.getLogger(__name__)
    try:
        return func()
    except Exception as e:
        _logger.debug("Safe call failed [%s]: %s", context, e, exc_info=True)
        return default
